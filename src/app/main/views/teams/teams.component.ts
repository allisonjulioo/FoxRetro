...
import { TeamServiceService } from 'src/app/services/services/team-serivice.service';
...
export class TeamsComponent implements OnInit {
  ...
  public async getTeams(): Promise<void> {
    this.loading = true;
    this.cards = await this.teamService.getAll().toPromise();
  ...
  public openModalTeam(team?: Teams) {
    const modalRef = this.modalService.open(ModalNewTeamComponent, this.modalOptions);
  ...
  public deleteTeam(team?: Teams): void {
    this.teamService.delete(team.id).subscribe(({ message }: { message: string }) => {
      console.log(message);
