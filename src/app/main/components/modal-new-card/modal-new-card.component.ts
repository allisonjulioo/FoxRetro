...
export class ModalNewCardComponent implements AfterViewInit, OnInit {
  @Input() column: Column;
  ...
  public saveCard() {
    ...
    this.cardsService
      .create({ ...this.newCardForm.value, column_id: this.column.id })
      .pipe(first())
      .subscribe(
        (res) => this.activeModal.close(this.newCardForm.value),
