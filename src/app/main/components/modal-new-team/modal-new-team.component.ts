...
export class ModalNewTeamComponent implements OnInit {
  ...
  loading: boolean;
  submitted: boolean;
  get a() { return this.teamForm.controls; }
  ...
  ngOnInit(): void {
    this.teamForm = this.fb.group({
      ...
      description: [this.team?.description, [Validators.required, Validators.minLength(3)]],
      logo: [this.team?.logo, [Validators.required, Validators.minLength(3)]]
    });
    this.getTeams();
  }
  public async getTeams(): Promise<void> {
    this.teams = await this.teamService.getAll().toPromise();
  }
  public saveTeam() {
